...
[[noreturn]] void assert_terminate();
...
[[noreturn]] inline void assert_terminate() {
    ::abort();
}
#endif
...
[[noreturn]] inline void do_assert_fail(char const* source_text_c, util::SourceLocation loc) {
    auto new_line = '\n';
    char text[] = "\033[31;1mASSERT\033[0m: ";
    assert_write(text, sizeof(text) - 1);

    auto source_text = cstring_to_utf8_view(source_text_c);
    assert_write(reinterpret_cast<char const*>(source_text.data()), source_text.size_bytes());
...
[[noreturn]] inline void do_binary_assert_fail(char const* source_text_c, T&& a, U&& b, util::SourceLocation loc) {
...
#ifndef DI_NO_ASSERT_ALLOCATION
    auto t = di::format::vpresent_encoded<container::string::Utf8Encoding>(
        ": {}: {}:{}:{}"_sv, di::format::make_format_args<di::format::FormatContext<container::string::Utf8Encoding>>(
                                 cstring_to_utf8_view(loc.function_name()), cstring_to_utf8_view(loc.file_name()),
                                 loc.line(), loc.column()));
    if (t) {
        assert_write(reinterpret_cast<char const*>(t->data()), t->size_bytes());
    }
    ...
    if constexpr (concepts::Formattable<T> && concepts::Formattable<U>) {
        auto s = di::format::vpresent_encoded<container::string::Utf8Encoding>(
            "{}"_sv, di::format::make_format_args<di::format::FormatContext<container::string::Utf8Encoding>>(a));
        auto t = di::format::vpresent_encoded<container::string::Utf8Encoding>(
            "{}"_sv, di::format::make_format_args<di::format::FormatContext<container::string::Utf8Encoding>>(b));

...
constexpr auto find_last_not_of(Str const& string, meta::EncodingCodePoint<Str> code_point) {
    return container::find_last_not_of(View(string::begin(string), string::end(string)), view::single(code_point))
        .begin();
}
...
constexpr auto find_last_not_of(Str const& string, Con&& container) {
    return container::find_last_not_of(View(string::begin(string), string::end(string)), util::forward<Con>(container))
        .begin();
}
}

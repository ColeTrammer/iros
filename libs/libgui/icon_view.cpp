...
void IconView::did_attach() {
    ...
    on<App::ViewRootChanged>([this](auto&) {
        rebuild_layout();
        invalidate();
    });
...
void IconView::rebuild_layout() {
    ...
    for (int r = 0; r < item_count; r++) {
        ...
        auto info = item->info(m_name_column, App::ModelItemInfo::Request::Text | App::ModelItemInfo::Request::Bitmap);
        m_items.add({
            info.bitmap(),
            info.text().value_or(""),
            {},
...
void IconView::uninstall_model_listeners(App::Model& model) {
    m_items.clear();
...
App::ModelItem* IconView::item_at_position(const Point& point) {
    for (auto& item : m_items) {

...
struct FindLastIfNotFunction {
    ...
    constexpr Option<IteratorForContainer<C>> operator()(C&& container, Pred predicate, Proj projection = {}) const {
        return Alg::find_if_not(reversed(forward<C>(container)), move(predicate), move(projection)).map([](auto it) {
            return (++it).base();
        });
    }
};

constexpr inline auto find_last_if_not = FindLastIfNotFunction {};
}

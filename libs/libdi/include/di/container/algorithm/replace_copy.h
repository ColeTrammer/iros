...
namespace detail {
    struct ReplaceCopyFunction {
        template<concepts::InputIterator It, concepts::SentinelFor<It> Sent, typename T, typename U, concepts::OutputIterator<U const&> Out,
        ...
        constexpr InOutResult<It, Out> operator()(It first, Sent last, Out output, T const& old_value, U const& new_value,
                                                  Proj proj = {}) const {
        ...
        constexpr InOutResult<meta::BorrowedIterator<Con>, Out> operator()(Con&& container, Out output, T const& old_value,
                                                                           U const& new_value, Proj proj = {}) const {
            return (*this)(container::begin(container), container::end(container), util::move(output), old_value, new_value,
                           util::ref(proj));
        }
    };
}

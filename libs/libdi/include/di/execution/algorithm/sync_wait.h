...
namespace sync_wait_ns {
    ...
    template<typename Result, concepts::Lock Lock>
    struct ReceiverT {
        struct Type {
        public:
            explicit Type(Result* result, RunLoop<Lock>* run_loop)
    ...
    template<typename Result, concepts::Lock Lock = sync::DumbSpinlock>
    using Receiver = meta::Type<ReceiverT<Result, Lock>>;
    ...
    struct Function : function::pipeline::EnablePipeline {
        ...
        concepts::SameAs<ResultType<Send>> auto operator()(Send&& sender) const {
            if constexpr (requires {
                ...
                auto operation = connect(util::forward<Send>(sender),
                                         Receiver<ResultType<Send>>(util::address_of(value.value), util::address_of(run_loop)));
                start(operation);

...
__attribute__((noreturn)) void _entry(struct initial_process_info *info, int argc, char **argv, char **envp) {
    ...
    struct dynamic_elf_object program = build_dynamic_elf_object(
        (const ElfW(Dyn) *) info->program_dynamic_start, info->program_dynamic_size / sizeof(ElfW(Dyn)), (uint8_t *) info->program_offset,
    ...
    struct dynamic_elf_object loader = build_dynamic_elf_object(
        (const ElfW(Dyn) *) info->loader_dynamic_start, info->loader_dynamic_size / sizeof(ElfW(Dyn)), (uint8_t *) info->loader_offset,

...
void gpt_partition_device(struct block_device *block_device) {
    ...
    struct gpt_header *gpt = create_temp_phys_addr_mapping(page->phys_addr) + (offset % PAGE_SIZE);
    ...
    free_temp_phys_addr_mapping(gpt);
    gpt = NULL;
    ...
    for (size_t i = 0; i < partition_entry_count; i++) {
        ...
        void *gpt_partition_base = create_temp_phys_addr_mapping(page->phys_addr);
        ...
        free_temp_phys_addr_mapping(gpt_partition_base);

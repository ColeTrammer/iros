...
Result<bool, Error> static safe_exists(PathView path) {
    return Ext::System::exists(path).transform_error([&](auto error) {
        return make_string_error("Failed to check if file `{}' exists: {}", path, error.message());
    });
}
...
Result<bool, Error> GitDownloadStep::should_skip(Context&, const Port& port) {
    return TRY(safe_exists(port.source_directory()));
}
...
Result<bool, Error> TarDownloadStep::should_skip(Context&, const Port& port) {
    return TRY(safe_exists(download_destination(port))) && TRY(safe_exists(signature_download_destination(port))) &&
           TRY(safe_exists(port.source_directory()));
}
...
Result<void, Error> TarDownloadStep::act(Context& context, const Port& port) {
    ...
    switch (m_kind) {
        case Kind::Gz: {
            ...
            if (!TRY(safe_exists(download_path))) {
            ...
            if (!TRY(safe_exists(signature_path))) {
            ...
            if (!TRY(safe_exists(target_source_directory))) {
                TRY(context.run_process(Process::command("tar", "-xzf", download_path, "--directory", base_directory)));
                TRY(context.run_process(
                    Process::command("mv", base_directory.join(m_source_directory_in_tarball.view()), target_source_directory)));
            }
            break;
        }
        default:
...
Path TarDownloadStep::download_destination(const Port& port) const {
    return port.base_directory().join("download.tar.{}", kind_to_string(m_kind));
}

Path TarDownloadStep::signature_download_destination(const Port& port) const {
    return port.base_directory().join("download.tar.{}.sig", kind_to_string(m_kind));
}
...
Result<bool, Error> PatchStep::should_skip(Context&, const Port& port) {
    for (auto& patch_file : m_patch_files) {
        if (!TRY(safe_exists(patch_marker_path(port, patch_file.view())))) {
            return false;
        }
    }
    return true;
...
Result<void, Error> PatchStep::act(Context& context, const Port& port) {
    return context.with_working_directory(port.source_directory(), [&]() -> Result<void, Error> {
        ...
        return Ext::stop_on_error(m_patch_files, [&](auto& patch_file) -> Result<void, Error> {
            auto marker_path = patch_marker_path(port, patch_file.view());
            if (TRY(safe_exists(marker_path))) {
                return {};
            }
            TRY(context.run_process(Process::command("git", "apply", patch_path(port, patch_file.view()))));
            return context.run_process(Process::command("touch", patch_marker_path(port, patch_file.view())));
        });
    });
}
...
Path PatchStep::patch_path(const Port& port, StringView patch_name) const {
    return port.definition_directory().join(patch_name);
}

Path PatchStep::patch_marker_path(const Port& port, StringView patch_name) const {
    return port.source_directory().join(".did_{}", patch_name);
}
...
Result<bool, Error> CMakeConfigureStep::should_skip(Context&, const Port& port) {
    return safe_exists(port.build_directory());
}
...
Result<void, Error> CMakeConfigureStep::act(Context& context, const Port& port) {
    ...
    auto toolchain_file = config.iros_source_directory().join("cmake/CMakeToolchain_{}.txt", config.target_architecture());
...
Result<void, Error> AutoconfConfigureStep::act(Context& context, const Port& port) {
    ...
    auto configure_script = port.source_directory().join("configure"_pv);
    auto build_directory = port.build_directory();

    auto arguments = NewVector<String> {};
    arguments.push_back(move(configure_script).into_string());
    auto settings = m_settings.clone();
    settings.try_emplace("--host", String(host));
    settings.try_emplace("--prefix", String(install_prefix));
...
Result<void, Error> CMakeInstallStep::act(Context& context, const Port& port) {
    auto enviornment = Enviornment::current().set("DESTDIR", String(context.config().iros_sysroot().data()));
...
Result<void, Error> CleanStep::act(Context& context, const Port& port) {
    return context.run_process(Process::command("rm", "-rf", port.base_directory()));
}
}

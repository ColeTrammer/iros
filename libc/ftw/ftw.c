...
static int add_dirent(char *path) {
    assert(path);
...
static int ftw_build_dirents(const char *path) {
    DIR *d = opendir(path);
    if (!d) {
        return -1;
    }
...
static int ftw_traverse(fn_t fn) {
    int depth = 0;
    while (size > 0) {

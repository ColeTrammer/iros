...
Result<Config, Error> Config::create() {
    ...
    auto port_json_directory = iros_source_directory.join_component("ports");
...
Ext::Path Config::port_json_for_port(const PortHandle& handle) const {
    return port_json_directory().join_component(String(handle)).join_component("port.json");
}
}

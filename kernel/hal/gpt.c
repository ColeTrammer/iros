...
void gpt_partition_device(struct block_device *block_device) {
    ...
    debug_log("Drive has GPT: [ %lu, %lu, %u, %u ]\n", block_device->device->device_number, gpt->partition_entry_start,
              gpt->parittion_entry_count, gpt->partition_entry_size);
    ...
    for (size_t i = 0; i < gpt->parittion_entry_count; i++) {
        ...
        if (!uuid_equals(gpt_partition->type_uuid, UUID_ZEROES)) {
            debug_log("GPT partition: [ %lu, %lu ]\n", gpt_partition->start, gpt_partition->end);

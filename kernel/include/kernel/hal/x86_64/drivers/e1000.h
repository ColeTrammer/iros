#ifndef _KERNEL_HAL_X86_64_DRIVERS_E1000_H
#define _KERNEL_HAL_X86_64_DRIVERS_E1000_H 1

#include <stdint.h>

#include <kernel/hal/x86_64/drivers/pci.h>

#define E1000_CTRL_REG     0x0000
#define E1000_STATUS_REG   0x0008
#define E1000_EEPROM_REG   0x0014
#define E1000_CTRL_EXT_REG 0x0018
#define E1000_CTRL_IMASK   0x00D0

#define E1000_TIPG_REG     0x0410      // Transmit Inter Packet Gap
#define E1000_ECTRL_SLU    0x40        //set link up

#define E1000_RCTRL        0x0100
#define E1000_RX_DESC_LO   0x2800
#define E1000_RX_DESC_HI   0x2804
#define E1000_RX_DESC_LEN  0x2808
#define E1000_RX_DESC_HEAD 0x2810
#define E1000_RX_DESC_TAIL 0x2818

#define E1000_TCTRL        0x0400
#define E1000_TX_DESC_LO   0x3800
#define E1000_TX_DESC_HI   0x3804
#define E1000_TX_DESC_LEN  0x3808
#define E1000_TX_DESC_HEAD 0x3810
#define E1000_TX_DESC_TAIL 0x3818

#define E1000_RCTL_EN                         (1 << 1)    // Receiver Enable
#define E1000_RCTL_SBP                        (1 << 2)    // Store Bad Packets
#define E1000_RCTL_UPE                        (1 << 3)    // Unicast Promiscuous Enabled
#define E1000_RCTL_MPE                        (1 << 4)    // Multicast Promiscuous Enabled
#define E1000_RCTL_LPE                        (1 << 5)    // Long Packet Reception Enable
#define E1000_RCTL_LBM_NONE                   (0 << 6)    // No Loopback
#define E1000_RCTL_LBM_PHY                    (3 << 6)    // PHY or external SerDesc loopback
#define E1000_RTCL_RDMTS_HALF                 (0 << 8)    // Free Buffer Threshold is 1/2 of RDLEN
#define E1000_RTCL_RDMTS_QUARTER              (1 << 8)    // Free Buffer Threshold is 1/4 of RDLEN
#define E1000_RTCL_RDMTS_EIGHTH               (2 << 8)    // Free Buffer Threshold is 1/8 of RDLEN
#define E1000_RCTL_MO_36                      (0 << 12)   // Multicast Offset - bits 47:36
#define E1000_RCTL_MO_35                      (1 << 12)   // Multicast Offset - bits 46:35
#define E1000_RCTL_MO_34                      (2 << 12)   // Multicast Offset - bits 45:34
#define E1000_RCTL_MO_32                      (3 << 12)   // Multicast Offset - bits 43:32
#define E1000_RCTL_BAM                        (1 << 15)   // Broadcast Accept Mode
#define E1000_RCTL_VFE                        (1 << 18)   // VLAN Filter Enable
#define E1000_RCTL_CFIEN                      (1 << 19)   // Canonical Form Indicator Enable
#define E1000_RCTL_CFI                        (1 << 20)   // Canonical Form Indicator Bit Value
#define E1000_RCTL_DPF                        (1 << 22)   // Discard Pause Frames
#define E1000_RCTL_PMCF                       (1 << 23)   // Pass MAC Control Frames
#define E1000_RCTL_SECRC                      (1 << 26)   // Strip Ethernet CRC
 
#define E1000_RCTL_BSIZE_256                  (3 << 16)
#define E1000_RCTL_BSIZE_512                  (2 << 16)
#define E1000_RCTL_BSIZE_1024                 (1 << 16)
#define E1000_RCTL_BSIZE_2048                 (0 << 16)
#define E1000_RCTL_BSIZE_4096                 ((3 << 16) | (1 << 25))
#define E1000_RCTL_BSIZE_8192                 ((2 << 16) | (1 << 25))
#define E1000_RCTL_BSIZE_16384                ((1 << 16) | (1 << 25))

#define E1000_CMD_EOP                         (1 << 0)    // End of Packet
#define E1000_CMD_IFCS                        (1 << 1)    // Insert FCS
#define E1000_CMD_IC                          (1 << 2)    // Insert Checksum
#define E1000_CMD_RS                          (1 << 3)    // Report Status
#define E1000_CMD_RPS                         (1 << 4)    // Report Packet Sent
#define E1000_CMD_VLE                         (1 << 6)    // VLAN Packet Enable
#define E1000_CMD_IDE                         (1 << 7)    // Interrupt Delay Enable
 
#define E1000_TCTL_EN                         (1 << 1)    // Transmit Enable
#define E1000_TCTL_PSP                        (1 << 3)    // Pad Short Packets
#define E1000_TCTL_CT_SHIFT                   4           // Collision Threshold
#define E1000_TCTL_COLD_SHIFT                 12          // Collision Distance
#define E1000_TCTL_SWXOFF                     (1 << 22)   // Software XOFF Transmission
#define E1000_TCTL_RTLC                       (1 << 24)   // Re-transmit on Late Collision
 
#define E1000_TSTA_DD                         (1 << 0)    // Descriptor Done
#define E1000_TSTA_EC                         (1 << 1)    // Excess Collisions
#define E1000_TSTA_LC                         (1 << 2)    // Late Collision
#define E1000_LSTA_TU                         (1 << 3)    // Transmit Underrun

#define E1000_NUM_RECIEVE_DESCS  32
#define E1000_NUM_TRANSMIT_DESCS 8

#define E1000_DESC_MIN_ALIGN 16

// Declared volatile b/c it this is accessed directly by the card
struct e1000_recieve_desc {
    volatile uint64_t addr;
    volatile uint16_t length;
    volatile uint16_t checksum;
    volatile uint8_t status;
    volatile uint8_t errors;
    volatile uint16_t special;
} __attribute__((packed));

struct e1000_transmit_desc {
    volatile uint64_t addr;
    volatile uint16_t length;
    volatile uint8_t cso;
    volatile uint8_t cmd;
    volatile uint8_t status;
    volatile uint8_t css;
    volatile uint16_t special;
} __attribute__((packed));

struct e1000_data {
    uintptr_t mem_io_phys_base;
    uint16_t io_port_base;

    uint8_t *rx_descs_unaligned;
    uint8_t *tx_descs_unaligned;

    struct e1000_recieve_desc *rx_descs;
    struct e1000_transmit_desc *tx_descs;

    uint8_t *rx_virt_addrs[E1000_NUM_RECIEVE_DESCS];
    uint8_t *tx_virt_addrs[E1000_NUM_TRANSMIT_DESCS];

    int current_rx;
    int current_tx;
};

void init_intel_e1000(struct pci_configuration *config);

#endif /* _KERNEL_HAL_X86_64_DRIVERS_E1000_H */
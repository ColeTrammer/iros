...
static struct vm_region *current_vm_list;
...
void init_vm_allocator(uintptr_t initrd_phys_start, uintptr_t initrd_phys_end) {
    ...
    current_vm_list = add_vm_region(current_vm_list, &kernel_text);
    ...
    current_vm_list = add_vm_region(current_vm_list, &kernel_rod);
    ...
    current_vm_list = add_vm_region(current_vm_list, &kernel_data);
    ...
    current_vm_list = add_vm_region(current_vm_list, &vga_buffer);
    ...
    current_vm_list = add_vm_region(current_vm_list, &initrd);
    ...
    current_vm_list = add_vm_region(current_vm_list, &kernel_heap);

    clear_initial_page_mappings();

    uintptr_t new_structure = create_paging_structure(current_vm_list);
    load_paging_structure(new_structure);
}

void *add_vm_pages_end(size_t n, uint64_t type) {
    struct vm_region *region = get_vm_region(current_vm_list, type);
    uintptr_t old_end = region->end;
    if (extend_vm_region_end(current_vm_list, VM_KERNEL_HEAP, n) < 0) {
...
void *add_vm_pages_start(size_t n, uint64_t type) {
    ...
    if (extend_vm_region_start(current_vm_list, VM_KERNEL_HEAP, n) < 0) {
        return NULL; // indicate there is no room
    }
...
void remove_vm_pages_end(size_t n, uint64_t type) {
    uintptr_t old_end = get_vm_region(current_vm_list, type)->end;
    if (contract_vm_region_end(current_vm_list, type, n) < 0) {
...
void remove_vm_pages_start(size_t n, uint64_t type) {
    uintptr_t old_start = get_vm_region(current_vm_list, type)->start;
    if (contract_vm_region_start(current_vm_list, type, n) < 0) {
        printf("%s\n", "Error: Removed to much memory");
        abort();
    }
    for (size_t i = 0; i < n; i++) {
...
struct vm_region *find_vm_region(uint64_t type) {
    return get_vm_region(current_vm_list, type);
}

struct vm_region *clone_kernel_vm() {
    if (current_vm_list == NULL) { return NULL; }

    struct vm_region *list = NULL;
    struct vm_region *iter = current_vm_list;
    while (iter != NULL) {

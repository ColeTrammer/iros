...
WindowManager::WindowManager(int fb, SharedPtr<Bitmap> front_buffer, SharedPtr<Bitmap> back_buffer)
    ...
    m_watcher = App::FileWatcher::create(nullptr);
    m_watcher->on<App::PathChangeEvent>({}, [this](auto& event) {
        load_palette(event.path());
    });
...
void WindowManager::add_window(SharedPtr<Window> window) {
    ...
    if (window->visible()) {
        if (!window->parent()) {
            ...
            if (window->type() != WindowServer::WindowType::Taskbar) {

...
void spawn_process(char **argv, uid_t uid, bool redirect) {
    int pid = fork();
    if (pid == 0) {
        if (uid != 0) {
            if (setuid(uid)) {
